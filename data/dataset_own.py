...
class SpeakerDataset(Dataset):
    def __init__(self, keys, data_dir, speaker_embedding_path, seg_len):
        ...
        self.speaker_embedding = torch.from_numpy(np.load(speaker_embedding_path))
    ...
    def __getitem__(self, index):
        ...
        current_mel = np.load(os.path.join(self.path, 'mels_4', "%s.npy" % self.fnames[index]))
        # current_mel = torch.from_numpy(current_mel)
