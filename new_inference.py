...
def main(
    ...
    speaker_emb = torch.from_numpy(np.load(speaker_embedding_path).astype(np.float32))
    ...
    src_emb = speaker_emb[src_speaker_id]

    # tar_embed
    tar_speaker_id = speaker2idx[src_speaker]
    tar_emb = speaker_emb[tar_speaker_id]
    ...
    final_result_path = result_path / src_mel_path.name.split(".")[0]
